...
int main() {
  ...
  while(~scanf("%d%d%d", &n, &m, &q)) {
    ...
    for(int i = 1; i <= n; i++)
    {
      add(1, 0, Max, l[i] + 1, r[i], arr[i]);
      if(par[i]) add(1, 0, Max, l[i], bpar[i], -arr[i]);
    }
    printf("Case #%d:\n", ++cnt);
    while(q--) {
      int a, b, c;
      ...
      c = __builtin_ctz(c);
      if(a == 1) {
        add(1, 0, Max, l[b] + 1, r[b], -arr[b] + c);
        if(par[b]) add(1, 0, Max, l[b], bpar[b], arr[b] - c);
        arr[b] = c;
      } else {
        int t = get(1, 0, Max, l[b], l[b] + 1);
        int y = get(1, 0, Max, l[root[b]], bpar[root[b]]);
        double z = c - t + y;
        printf("%.3f\n", Z * z);
      }
    }
