...
void dfs2(int v,int prnt,int e){
    ...
    auto w = ms[rv].lower_bound(mp(edges[e].F,0));
    if(w==ms[rv].end()) {
        LOG;
        ans[e] = ms[rv].rbegin()->F -1;
        return ;
    }
...
int main() {
    ...
    F0R(i,1,m) {
        ...
        int a = max(find_max(v,w),find_max(u,w));
        if( a ) {
            ans[i] = a-1;
            FOR(i,0,2){ 
                if(a < ::m[v][h[w]]) continue;
                int www = edges[i].F;
                auto x = ms[v].find(mp(::m[v][h[w]],h[w]));
                if(x!=ms[v].end())
                    ms[v].erase(x);
                ms[v].insert(mp(www,h[w]));
                ::m[v][h[w]] = edges[i].F;
                swap(u,v);
            }
        }
